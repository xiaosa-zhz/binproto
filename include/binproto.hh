...
public:
    ...
    constexpr binary_view(std::span<std::byte> raw) noexcept : raw(raw) {}

private:
    template<std::meta::info Mem>
    static constexpr bool is_member_accessible
        = (is_class_type(^^value_type)) && (is_nonstatic_data_member(Mem))
            && (requires (value_type& v) { v.[:Mem:]; });

public:
    template<std::meta::info Mem>
        requires is_member_accessible<Mem> && (!is_bit_field(Mem))
    constexpr binary_view<typename [:type_of(Mem):], endian, packed> subview() const noexcept {
    ...
    constexpr std::error_code write(const value_type& value) noexcept {
        static constexpr auto layout = details::layout_of<value_type, endian, packed>;
        if (raw.size() < layout.total_size) {
            return std::make_error_code(std::errc::result_out_of_range);
        }
    ...
    template<std::meta::info Mem>
        requires is_member_accessible<Mem>
    ...
    template<std::meta::info Mem>
        requires is_member_accessible<Mem>
    constexpr std::error_code write(typename [:add_const(type_of(Mem)):]& value) noexcept {
        if constexpr (is_bit_field(Mem)) {
            static constexpr std::size_t offset
                = details::get_overall_offset_of_member(^^value_type, endian, packed, Mem);
            static constexpr std::size_t group_size
