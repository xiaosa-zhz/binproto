...
int run_bitfield_tests() {
    // --- Flags: single-byte group, wire format + whole-struct roundtrip ---
    {
        ...
        Flags f{5, 25};
        if (!view.write(f)) {
            std::println("flags LE write failed");
            return 1;
        }
        ...
        Flags r{};
        if (!view.read(r)) {
            std::println("flags LE read failed");
            return 1;
        }
    ...
    // --- Flags: member-wise read/write must not clobber the sibling field ---
    {
        ...
        if (!view.write<^^Flags::a>(std::uint8_t{5})) return 1;
        if (!view.write<^^Flags::b>(std::uint8_t{25})) return 1;
        bool wire_ok = std::to_integer<unsigned>(storage[0]) == 0xCD;

        std::uint8_t a = 0, b = 0;
        if (!view.read<^^Flags::a>(a)) return 1;
        if (!view.read<^^Flags::b>(b)) return 1;
    ...
    // --- SignedBit: negative values must sign-extend on read ---
    {
        ...
        for (int v : {-3, -1, 2}) {
            ...
            if (!view.read(r)) return 1;
            int m = 0;
            if (!view.read<^^SignedBit::x>(m)) return 1;
            ok = ok && r.x == v && m == v;
        }
    ...
    // --- BitDerived: bit-field inherited from a base class ---
    {
        ...
        std::uint8_t a = 0;
        if (!view.read<^^BitDerived::a>(a)) return 1;
        if (!view.write<^^BitDerived::b>(std::uint8_t{17})) return 1;
        std::uint8_t b = 0;
        if (!view.read<^^BitDerived::b>(b)) return 1;
        ok = ok && a == 5 && b == 17;
    ...
    // --- U64Bits: a full 64-bit group, both endians ---
    {
        ...
        if (!view_be.write<^^U64Bits::a>(v.a)) return 1;
        if (!view_be.write<^^U64Bits::b>(v.b)) return 1;
    ...
    // --- AnonPad: anonymous non-zero-width bit-field, 12-bit group, both endians ---
    {
        ...
        std::uint8_t mb = 0;
        if (!view_le.read<^^AnonPad::b>(mb)) return 1;
        ok = ok && mb == 25;
    ...
    // --- MultiZero: multiple zero-width bit-fields, three independent groups ---
    {
        ...
        if (!view_le.read<^^MultiZero::a>(ma)) return 1;
        if (!view_le.read<^^MultiZero::b>(mb)) return 1;
        if (!view_le.read<^^MultiZero::c>(mc)) return 1;
    ...
    // --- BitDerived2: bit-field inherited from a base that has an anonymous bit-field ---
    {
        ...
        std::uint8_t a = 0, b = 0;
        if (!view.read<^^BitDerived2::a>(a)) return 1;
        if (!view.read<^^BitDerived2::b>(b)) return 1;
        if (!view.write<^^BitDerived2::b>(std::uint8_t{17})) return 1;
        std::uint8_t b2 = 0;
        if (!view.read<^^BitDerived2::b>(b2)) return 1;
    ...
    // --- DeepDerived: inheritance + zero-width + anonymous bit-field + scalar ---
    {
        ...
        std::uint32_t d = 0;
        if (!view.read<^^DeepDerived::d>(d)) return 1;
        if (!view.write<^^DeepDerived::d>(std::uint32_t{0xDEADBEEF})) return 1;
        std::uint32_t d2 = 0;
        if (!view.read<^^DeepDerived::d>(d2)) return 1;
    ...
    // --- RestartAligned pack(8): zero-width terminated group restarts on an aligned byte ---
    {
        ...
        std::uint32_t mb = 0;
        if (!view.read<^^RestartAligned::b>(mb)) return 1;
    ...
    // --- Bounds checking on bit-field access ---
    {
        ...
        bool ok = !view.read(f)
               && !view.read<^^Flags::a>(a)
               && !view.write(f)
               && !view.write<^^Flags::a>(std::uint8_t{1});
